...
This module is intentionally self-contained and imported by launcher.py before the
Qt application starts. It augments the existing webcam source without changing
VisoMaster's core processing pipeline: screen capture is exposed as a webcam-like
live source backed by a small VideoCapture-compatible adapter.
"""
...
def enumerate_monitors() -> list[MonitorInfo]:
    """Return physical Windows monitor rectangles in stable enumeration order."""
    if not IS_WINDOWS:
        return []
...
class ScreenCaptureSource:
    """PIL/Win32-backed screen grabber with a VideoCapture-like interface."""
...
def _screen_thumbnail(main_window):
    ...
    source = ScreenCaptureSource(
        monitor_index=_monitor_index_from_control(main_window),
        fps=_fps_from_control(main_window),
    )
...
def _install_settings() -> None:
    ...
    if "ScreenCaptureMonitorSelection" not in general:
        monitors = enumerate_monitors()
        options = [m.label for m in monitors] or ["Monitor 1"]
        general["ScreenCaptureMonitorSelection"] = {
            "level": 1,
    ...
    if "ScreenCaptureFpsSlider" not in general:
        general["ScreenCaptureFpsSlider"] = {
            "level": 1,
...
def _install_target_media_source() -> None:
    ...
    def load_media(self):
        ...
        try:
            monitor_index = _monitor_index_from_control(main_window)
            fps = _fps_from_control(main_window)
            source = ScreenCaptureSource(monitor_index=monitor_index, fps=fps)
            ok, frame_bgr = source.read()
            if not ok or frame_bgr is None:
            ...
            if main_window.control.get("SendVirtCamFramesEnableToggle"):
...
def _install_video_processor_hooks() -> None:
    ...
    def stop_processing(self, *args, **kwargs):
        is_screen = getattr(self, "_screen_capture_source", None) is not None
        result = original_stop(self, *args, **kwargs)
        if is_screen and getattr(self, "file_type", None) == "webcam":
            try:
                source = ScreenCaptureSource(
                    monitor_index=_monitor_index_from_control(self.main_window),
                    fps=_fps_from_control(self.main_window),
                )
