...
def _load_screen(self):
    """Load the desktop source without entering VideoProcessor's webcam path."""
    mw = self.main_window
    ...
    try:
        ...
        # Never use "webcam" here: that value invokes the normal webcam startup
        # path, which opens cv2.VideoCapture and overwrites our screen source.
        vp.file_type = "screen"
        vp.fps = source.fps
...
def add_screen_capture_card(mw):
    ...
    list_view_actions.add_media_thumbnail_button(
        mw, widget_components.TargetMediaCardButton, mw.targetVideosList, mw.target_videos,
        image, media_path="Screen Capture", file_type="video", media_id="screen-capture"
    )
...
def open_screen_capture_window(mw):
    ...
    fps.setValue(30)
    for m in monitors:
        monitor.addItem(m.label, m.index)
